...
void RNP::GMRES_alloc(size_t n, const GMRES_params &params, LinearSolverWorkspace *workspace){
	...
	workspace->zwork = new std::complex<double>[(2+m+n)*(m+1)+m+n];
...
int RNP::GMRES(
	...
	void (*P)(const std::complex<double>*, std::complex<double>*, void*),
	void *Pdata
){
	...
	std::complex<double> *tmp = _work->zwork; // length n
	std::complex<double> *sn = tmp + n; // length mxm
	...
	while(num_multmv <= ls_params.max_mult && iter <= maxit && rnrm > eps1){
		...
		while(m < mxm && rnrm > eps1 && num_multmv <= ls_params.max_mult){
			...
			if(NULL != P){
				P(&v[0+(m+1)*n], tmp, Pdata);
				RNP::TBLAS::Copy(n, tmp, 1, &v[0+(m+1)*n], 1);
			}
		...
		if(NULL != P){
			P(&v[0+1*n], tmp, Pdata);
			RNP::TBLAS::Copy(n, tmp, 1, &v[0+1*n], 1);
		}
		RNP::TBLAS::Copy(n, b, 1, &v[0+0*n], 1);
