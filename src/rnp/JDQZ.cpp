...
struct jdqz_preconditioner_data{
	size_t n;
	void (*preconditioner)(const std::complex<double> &alpha, const std::complex<double> &beta, const std::complex<double> *, std::complex<double> *, void *);
...
void jdqz_preconditioner(const std::complex<double> *x, std::complex<double> *y, void *_data){
	jdqz_preconditioner_data *data = reinterpret_cast<jdqz_preconditioner_data*>(_data);
	if(NULL != data->preconditioner){
		data->preconditioner(data->alpha, data->beta, x, y, data->preconditioner_data);
	}
	RNP::TBLAS::MultMV<'C'>(data->n, data->nq, 1., data->q, data->n, y, 1, 0., data->work, 1);
	RNP::TLASupport::LUSolve<'N'>(data->nq, 1, data->invqkz, data->ldqkz, data->ipiv, data->work, data->ldqkz);
	RNP::TBLAS::MultMV<'N'>(data->n, data->nq, -1., data->kz, data->n, data->work, 1, 1., y, 1);
}
...
void RNP::JDQZ_GMRES_adapter(
	...
	double tol,
	size_t &max_mult,
	void (*jdqz_precon)(const std::complex<double> *x, std::complex<double> *y, void *precon_data),
	void *jdqz_precon_data,
	void *_data
){
	jdqz_linsolve_data *data = reinterpret_cast<jdqz_linsolve_data*>(_data);
	RNP::GMRES_params params;
	params.m = 10;
	LinearSolverParams ls_params = data->ls_params;
	ls_params.max_mult = max_mult;
	ls_params.tol = tol;
	ls_params.max_iterations = 5;
	RNP::TBLAS::Scale(n, -1., b, 1);
	RNP::GMRES(n, Op, b, x, ls_params, params, data->workspace, op_data, jdqz_precon, jdqz_precon_data);
	max_mult = ls_params.max_mult;
}
...
int RNP::JDQZ(
	...
	bool (*EvalSorter)(const std::complex<double> &, const std::complex<double> &, const std::complex<double> &, const std::complex<double> &, void*),
	void (*preconditioner)(const std::complex<double> &alpha, const std::complex<double> &beta, const std::complex<double> *, std::complex<double> *, void *),
	void (*randvec)(std::complex<double> *, void *),
	void (*orth)(std::complex<double> *, void *),
	...
	*/
	...
	while(k < kmax && step < params.max_iters){
		...
		if(j == 0){ // set v to initial value v0
			...
			*/
			randvec(&v[0+0*n], randvec_data);
			randvec(&w[0+0*n], randvec_data);
#ifdef DEBUG_JDQZ
...
//			RNP::IO::PrintVector(n, &v[0+0*n]) << std::endl;
			...
			precon_data.nq = k+1;
			precon_data.alpha = zalpha;
			precon_data.beta = zbeta;
			if(j < jmin){
				linsolve_max_mult = 1;
			}
...
//		RNP::IO::PrintMatrix(j, j, vsr, jmax) << std::endl;
		...
		while(found){
			...
			if(NULL != preconditioner){
				preconditioner(zalpha, zbeta, &z[0+k*n], &kz[0+k*n], preconditioner_data);
			}
